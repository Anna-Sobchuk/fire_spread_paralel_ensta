    ...
    if (rank == 0) {
        ...
        try {
            ...
            while (true) {
                ...
                // Parallelize the update function using OpenMP
                bool updated = false;
                #pragma omp parallel
                {
                    #pragma omp single
                    {
                        updated = simu.update();  // Call the update function
                    }
                }

                if (!updated) break;  // Exit the loop if the simulation is done
            ...
            std::cout << "Temps moyen d'avancement en temps: " << total_advancement_time.count() / total_steps << " s" << std::endl;
        }
        catch (const std::exception& e) {
