...
Model::Model(double t_length, unsigned t_discretization, std::array<double,2> t_wind,
    ...
    int t_local_start, int t_local_end)
:   m_local_start_row(t_local_start),
    m_local_end_row(t_local_end),
    m_length(t_length),
    m_distance(-1),
    ...
    m_local_vegetation(),  // Initialisation différée
    m_local_fire()         // Initialisation différée
{
if (t_discretization == 0) {
    throw std::range_error("Le nombre de cases par direction doit être plus grand que zéro.");
}

m_distance = m_length / static_cast<double>(m_geometry);

// Taille locale avec cellules fantômes
const int local_rows = m_local_end_row - m_local_start_row;
m_local_vegetation.resize(local_rows * m_geometry, 255u);
m_local_fire.resize(local_rows * m_geometry, 0u);

// Initialisation du foyer uniquement si dans la tranche locale
if(t_start_fire_position.row >= m_local_start_row && 
   t_start_fire_position.row < m_local_end_row) 
{
    const int local_row = t_start_fire_position.row - m_local_start_row;
    const std::size_t index = local_row * m_geometry + t_start_fire_position.column;
    m_local_fire[index] = 255u;
    m_fire_front[index] = 255u;
}

// Calcul des paramètres de vent (inchangé)
constexpr double alpha0 = 4.52790762e-01;
constexpr double alpha1 = 9.58264437e-04;
constexpr double alpha2 = 3.61499382e-05;

m_wind_speed < t_max_wind ?
    p1 = alpha0 + alpha1*m_wind_speed + alpha2*(m_wind_speed*m_wind_speed) :
    p1 = alpha0 + alpha1*t_max_wind + alpha2*(t_max_wind*t_max_wind);

p2 = 0.3;

// Gestion direction vent (optimisée)
const double wx = m_wind[0]/t_max_wind;
const double wy = m_wind[1]/t_max_wind;

alphaEastWest = (wx > 0) ? std::abs(wx) + 1 : 1. - std::abs(wx);
alphaWestEast = (wx > 0) ? 1. - std::abs(wx) : std::abs(wx) + 1;

alphaSouthNorth = (wy > 0) ? std::abs(wy) + 1 : 1. - std::abs(wy);
alphaNorthSouth = (wy > 0) ? 1. - std::abs(wy) : std::abs(wy) + 1;
}
// --------------------------------------------------------------------------------------------------------------------
void
Model::exchange_ghosts()
{
    MPI_Request requests[4];
    int prev_rank, next_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &prev_rank);
    next_rank = prev_rank + 1;
    prev_rank -= 1;

    // Échange des lignes fantômes nord/sud
    if(prev_rank >= 0) {
        MPI_Isend(&m_local_fire[m_geometry], m_geometry, MPI_UNSIGNED_CHAR, 
                 prev_rank, 0, MPI_COMM_WORLD, &requests[0]);
        MPI_Irecv(&m_local_fire[0], m_geometry, MPI_UNSIGNED_CHAR,
                 prev_rank, 0, MPI_COMM_WORLD, &requests[1]);
    }
    
    if(next_rank < MPI_COMM_WORLD.Get_size()) {
        MPI_Isend(&m_local_fire[(m_local_end_row - m_local_start_row - 2)*m_geometry], 
                 m_geometry, MPI_UNSIGNED_CHAR, next_rank, 0, 
                 MPI_COMM_WORLD, &requests[2]);
        MPI_Irecv(&m_local_fire[(m_local_end_row - m_local_start_row - 1)*m_geometry],
                 m_geometry, MPI_UNSIGNED_CHAR, next_rank, 0,
...
Model::update()
{
    ...
    // Nouveau : Appel à l'échange de fantômes
    exchange_ghosts();
