...
{
public:
    struct LexicoIndices { unsigned row, column; };
    ...
    Model(double t_length, unsigned t_discretization, std::array<double,2> t_wind,
          ...
          int t_local_start = 0, int t_local_end = 0);
    ...
    std::vector<std::uint8_t> vegetal_map() const;
    std::vector<std::uint8_t> fire_map() const;
...
private:
    ...
    int m_local_start_row;
    int m_local_end_row;
    int m_ghost_layers = 1;
    std::vector<std::uint8_t> m_local_vegetation;
    std::vector<std::uint8_t> m_local_fire;
    MPI_Comm m_compute_comm;
    ...
    std::vector<std::uint8_t> /*m_vegetation_map*/ m_local_vegetation, /*m_fire_map*/ m_local_fire_map;
    double p1{0.}, p2{0.};
